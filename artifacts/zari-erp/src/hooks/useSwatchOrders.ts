...
export function useSwatchOrderList(params: { search: string; status: string; priority: string; chargeable: string; inhouse: string; tag:string; page: number; limit: number }) {
  return useQuery({
    queryKey: [QK, params],
    queryFn: () =>
      customFetch<{ data: SwatchOrderRecord[]; total: number; page: number; limit: number }>(
        `${BASE}?search=${encodeURIComponent(params.search)}&status=${params.status}&priority=${params.priority}&chargeable=${params.chargeable}&inhouse=${params.inhouse}&tag=${params.tag}&page=${params.page}&limit=${params.limit}`,
      ),
