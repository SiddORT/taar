...
export default function FabricMaster() {
  ...
  const columns: Column[] = [
    ...
    { key: "fabricCode", label: "Code", render: (r) => <span className="font-mono font-semibold text-cyan-900">{asFab(r).fabricCode}</span> },
    { key: "fabricType", label: "Type", render: (r) => <span className="text-slate-500">{asFab(r).fabricType}</span> },
    { key: "quality", label: "Quality", render: (r) => <span className="text-slate-500">{asFab(r).quality}</span> },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-500">{asFab(r).colorName}</span>
        </div>
      ),
    },
    ...
    { key: "hsnCode", label: "HSN Code", render: (r) => <span className="font-mono text-slate-500">{asFab(r).hsnCode}</span> },
    ...
    { key: "updatedAt", label: "Updated At", render: (r) => <span className="text-gray-500 whitespace-nowrap">{formatDate(asFab(r).updatedAt)}</span> },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2">
          <button onClick={() => openEdit(asFab(r))} className="p-1.5 rounded-lg text-gray-400 hover:text-slate-500 hover:bg-gray-100 transition-colors" title="Edit">
                      ...
                      {selectedVendors.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-1">
                          {selectedVendors.map(name => (
                            <span key={name} className="inline-flex items-center gap-1 rounded-full bg-amber-50 border border-[#C9B45C]/40 px-2.5 py-1 text-xs font-medium text-slate-500">
                              {name}
                              ...
                              {availableVendors.length === 0 ? (
                                ...
                                availableVendors.map(name => (
                                  <button key={name} type="button"
                                    ...
                                    className="flex w-full items-center px-4 py-2 text-sm text-slate-500 hover:bg-amber-50/60 hover:text-cyan-900 transition-colors text-left"
                                  >
                                    {name}
                                  </button>
                                ))
