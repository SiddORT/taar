...
export default function DepartmentMaster() {
  ...
  const columns: Column[] = [
    ...
    { key: "name", label: "Department Name", render: (r) => <span className="font-medium text-cyan-900">{asRec(r).name}</span> },
      ...
      key: "actions", label: "Actions", render: (r) => {
        ...
        return (
          <div className="flex gap-2">
            <button onClick={() => openEdit(rec)} className="p-1.5 rounded-lg text-gray-400 hover:text-slate-500 hover:bg-gray-100 transition-colors" title="Edit"><Pencil size={15} /></button>
  ...
  return (
    <AppLayout username={user.username} role={user.role} onLogout={handleLogout} isLoggingOut={logoutMutation.isPending}>
      <div className="max-w-screen-xl mx-auto space-y-5">
        <MasterHeader title="Department Master" onAdd={openCreate} addLabel="Add Department" />
      ...
      <MasterFormModal open={modalOpen} onClose={() => setModalOpen(false)}
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <button type="button" onClick={() => setForm(f => ({ ...f, isActive: !f.isActive }))}
            className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-900/20 ${form.isActive ? "bg-cyan-900" : "bg-gray-300"}`}
