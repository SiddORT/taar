...
/**
 * Generate a PR number in format PR-YY-XXXX
 */
...
/**
 * Get already received quantity for a BOM row from a PO
 */
...
/**
 * Get purchase order item by inventory ID
 */
...
export async function seedPurchaseReceipts(count: number = 0): Promise<void> {
  console.log(`\n📦 Starting PurchaseReceiptSeeder...`);

  // 1. Fetch ALL purchase orders (not just eligible ones)
  ...
  console.log(`   ✅ Found ${purchaseOrders.length} purchase orders (excluding Completed/Cancelled)`);
  ...
  for (const po of ordersToProcess) {
    ...
    if (!eligibleStatuses.includes(po.status)) {
      ...
      statusUpdated++;
      // Update the po object status for subsequent steps
      po.status = 'Approved';
    }
    ...
    for (const bomItem of bomItems) {
      ...
      // Get already received quantity for this BOM row
      ...
      // Generate random receipt quantity (1-50% of remaining, but at least 1)
      ...
      // Determine price - use target price from BOM or fallback to inventory average
      const unitPrice = parseFloat(bomItem.targetPrice || inventoryItem.averagePrice || '0');

      // Get vendor - use PO vendor or random
      ...
      try {
        ...
        await db.transaction(async (tx) => {
          ...
          // FIX: Use a type assertion to bypass TypeScript error
  ...
  console.log(`\n✅ PurchaseReceiptSeeder completed! Created ${totalReceipts} receipts, skipped ${skipped}, failed ${failed}.`);
  if (statusUpdated > 0) {
...
if (isMainModule) {
  ...
  seedPurchaseReceipts(count).catch(console.error);
}